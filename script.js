  ...
  try {
    ...
    playerImage.forEach((image, index) => {
      setTimeout(() => {
        ...
        } else if (action(image, "kick") && opponentBar <= 100) {
          audio.kick.play(); // play kick audio file
          if (playerXaxis >= opponentXaxis - 140) {
            audio.hit.play();
            if (!isOpponentBlocked) {
              opponentBar += 1; // reduce 5 points from opponent health
              playerAttackCount++;
            }
          }
      ...
      setTimeout(() => {
        ...
        if (action(image, "punch") && playerBar > 0) {
          if (opponentXaxis <= playerXaxis + 170) {
            audio.hit.play();
            if (!isPlayerBlocked) playerBar -= 0.5; // reduce 2 points from player health
          } else audio.punch.play();
        } else if (action(image, "kick") && playerBar > 0) {
          audio.kick.play();
          if (opponentXaxis <= playerXaxis + 140) {
            audio.hit.play();
            if (!isPlayerBlocked) playerBar -= 1; // reduce 5 points from player health
          }
        }
    ...
    setTimeout(
      callback,
      (playerImage.length <= opponentImage.length
        ? playerImage.length
        : opponentImage.length) * 100
    );
...
function startGame() {
  ...
  let selectAnimation = () =>
    playerMovement == null ? "idle" : playerMovement;
  ...
  document.getElementsByClassName("bar-layer")[0].style.backgroundColor =
    "green";
  ...
  let executeAction = () => {
    ...
    playerHealthBar.style.width = String(playerBar) + "%";
    opponentHealthBarBg.style.width = String(opponentBar) + "%";
  ...
  document.addEventListener("keydown", (event) => {
    switch (event.key) {
      case "d":
        ...
        break;
      case "a":
        playerMovement = "backward";
        break;
      case " ":
        playerMovement = "block";
    }
