...
let loadImages = (callback) => {
	let playerImages = { idle: [], punch: [], kick: [], forward: [], backward: [], block: [], release: [] };
	let opponentImages = { idle: [], punch: [], kick: [], forward: [], backward: [], block: [], release: [] };

	let imagesToLoad = 0;

	["idle", "punch", "kick", "forward", "backward", "block", "release"].forEach(
		(animation) => {
...
let animate = (context, playerImages, opponentImages, playerAnimation, oppenentAnimation, callback) => {
	...
	for (let index = 0; index < playerImage.length; index++) {
		setTimeout(() => {
			if (playerAction(playerImage[index], "forward") && playerXaxis <= opponentXaxis - 130) {
				playerXaxis += 10; // forward movement
			} else if (playerAction(playerImage[index], "backward") && playerXaxis > -50) {
				playerXaxis -= 10; // backward movement
			}

			// player attack
			if (playerAction(playerImage[index], "punch") && opponentBar < 500) {
			...
			} else if (playerAction(playerImage[index], "kick") && opponentBar < 500) {
				audio.kick.play();
			...
			let blocking = () => {
				loadImage(imagePath("player", "block", 5), )
			}
			context.drawImage(playerImage[index], playerXaxis, 0, 500, 500);
...
function ready() {
	...
	alert("Hold A to move backward\nHold D to move forward\nHold Space to block");
	alert("Press P to punch\nPress L to Kick");
	...
	loadImages((playerImages, opponentImages) => {
		...
		document.addEventListener("keyup", (event) => {
			switch (event.key) {
				...
				case " ": playerBlock = false;		playerAnimationQueue.push("release");
			}
